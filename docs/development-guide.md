...
- ⏳ 完善 `文稿` 模块基础功能
  - 🟡 **中优先级** - 体验增强：
    1. ✅ 章节状态管理优化：取消发布、批量发布、状态图标展示（已完成）
    2. ✅ 自动保存优化：离开页面确认、保存状态提示、切换章节前保存（已完成）
       - 离开页面确认：有未保存内容时拦截页面离开
       - 保存状态UI：实时显示保存中/未保存/已保存状态
       - 切换章节自动保存：切换前先保存当前内容，失败时给予确认选项
       - 内容变更检测：只在内容真正改变时才发送保存请求
       - 时间格式中文化：使用dayjs中文locale显示相对时间
    3. 封面管理：上传封面、展示缩略图
  - 🟢 **低优先级** ：
    1. 版本历史查看和回滚：展示历史版本列表，支持预览和回滚（后端已完成数据保存）
    2. 统计分析面板：字数趋势图、写作习惯分析、章节字数分布
    3. 关联设定详情展示：设定卡片预览、跳转到设定详情页
    4. 进度里程碑提示：完成度提示、徽章展示
