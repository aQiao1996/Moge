...
- ✅ 实现 `文稿` 模块（Phase 1 基础功能 + Phase 2 AI 增强）
  - 后端：完整的 CRUD API、从大纲创建、卷章管理、内容保存、版本历史、章节发布、字数统计、设定关联、排序接口
  - 前端：文稿列表/详情页、创建/编辑对话框、卷章树、编辑器、自动保存、侧边栏、最后编辑位置跳转
  - AI 功能：续写/润色/扩写接口及 UI、设定上下文注入

**待完成**:

- ⏳ 完善 `文稿` 模块基础功能
  - 🔴 **高优先级** - 影响基础可用性：
    1. 前端拖拽排序 UI：实现卷和章节的拖拽排序（后端 API 已完成）
    2. 编辑器选中文本功能：支持对选中文本进行 AI 润色/扩写
  - 🟡 **中优先级** - 体验增强：
