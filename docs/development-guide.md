...
- ✅ 实现 `文稿` 模块 (Phase 1: 基础功能) - 后端完成 ✅ 前端部分完成 ⏳
  ...
  - ⏳ 前端实现：
    ...
    - ✅ 编辑卷/章节功能：
      - ✅ 编辑卷对话框（EditVolumeDialog）：编辑卷标题和描述
      - ✅ 编辑章节对话框（EditChapterDialog）：编辑章节标题
...
- ⏳ 修复和完善 `文稿` 模块已知问题（优先级从高到低）：
  - 🔴 **高优先级** - 影响核心功能，需优先解决：
    ...
    5. ⏳ 修复编辑器选中文本功能：
       ...
       - 使 AI 润色和扩写功能能够针对选中文本工作
  - 🟡 **中优先级** - 提升用户体验：6. ⏳ 实现版本回滚功能：
    - 后端接口：`GET /manuscripts/chapters/:id/versions`（获取版本历史列表）
    - 后端接口：`POST /manuscripts/chapters/:id/rollback`（回滚到指定版本）
    - 前端 UI：在编辑器页面添加「版本历史」按钮，展示历史版本列表
    - 支持预览历史版本内容，确认后执行回滚
    7. ⏳ 完善章节状态管理：
       - 添加「取消发布」功能（将状态从 PUBLISHED 改回 DRAFT）
       - 添加批量发布功能（选择多个章节同时发布）
       - 在卷章树中用图标或徽章标识章节发布状态
    8. ⏳ 优化自动保存逻辑：
       - 添加离开页面前的确认提示（如果有未保存的内容）
       - 在编辑器顶部添加「草稿保存中」的明显提示（加载动画）
       - 切换章节时先保存当前章节内容
    9. ⏳ 实现封面管理：
       - 创建/编辑文稿时支持上传封面图片
       - 文稿列表和详情页显示封面缩略图
       - 集成图片上传服务（如 OSS、本地存储）
  - 🟢 **低优先级** - 锦上添花：10. ⏳ 完善「关联设定」Tab UI：- 展示已关联的设定详细信息（不只是 ID）- 支持查看设定详情卡片（悬浮预览）- 支持点击跳转到设定详情页 11. ⏳ 完善「统计分析」Tab：- 显示每日字数趋势图（折线图）- 显示章节字数分布（柱状图）- 显示写作活跃时间分布（热力图）- 计算平均每日字数、更新频率等指标 12. ⏳ 添加进度里程碑提示：- 当完成进度达到 25%、50%、75%、100% 时显示祝贺提示 - 在文稿详情页展示里程碑徽章 13. ⏳ 改进 AI 功能的错误提示：- 区分不同类型的错误（网络错误、AI 服务错误、权限错误等）- 显示更友好的错误信息和解决建议 - 添加重试按钮
- ⏳ 实现 `工作台` 模块
  - 最近项目列表
  - 统计概览卡片
  - 创作目标/待办清单
